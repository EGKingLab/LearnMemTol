...
pp <- ggplot(L.plot.s, aes(x=LearnPowerTrans,y=Memory_Mean, color=Haplotype)) +
  geom_point(alpha = 1/3, size=1) +
  geom_point(data = effs.dat, aes(x=eLearn,y=eMem,fill=Haplotype), size=3,pch=21, color="black") +
  geom_segment(data = effs.dat, aes(x=eLearn-LearnSE, xend=eLearn + LearnSE, y=eMem, yend=eMem), color = 'black') +
  geom_segment(data = effs.dat, aes(x=eLearn, xend=eLearn, y=eMem-MemSE, yend=eMem+MemSE), color = 'black') +
  #geom_point(data= L.plot.s.pool, aes(x=LearnPowerTrans, y=Memory_Mean, fill=Haplotype, shape=pool), color="black")+
  scale_shape_manual(values=c(24,25))+
  scale_fill_manual(values=color.8[2:8])+
  ylab("Memory") +
  xlab(expression(Learning^3)) +
  annotate("text", label=qnames[ii], x= 0.1, y=0.8, fontface=2) +
  theme(legend.position="none") +
...
ggsave(pp, file="../Plots/EffectPlot_Q4_tt.pdf", width=8/3, height=2.5)

L.plot.s <- subset(L.plot.s, Haplotype %in% c("A2","A5"))
effs.dat <- subset(effs.dat, Haplotype %in% c("A2","A5"))
...
pp <- ggplot(L.plot.s, aes(x=LearnPowerTrans,y=Memory_Mean, color=Haplotype)) +
  ...
  #geom_point(data= L.plot.s.pool, aes(x=LearnPowerTrans, y=Memory_Mean, fill=Haplotype, shape=pool), color="black")+
  scale_shape_manual(values=c(24,25))+
  scale_fill_manual(values=color.8[c(2,5)])+
  scale_color_manual(values=color.8[c(2,5)])+
  ylab("Memory") +
  xlab(expression(Learning^3)) +
  xlim(c(0,0.82)) +
  ylim(c(-0.02,0.88)) +
...
ggsave(pp, file="../Plots/EffectPlot_Q4_tt_2.pdf", width=8/3, height=2.5)
...
#Effect QTL for Thermal Tolerance 
```{r}
...
Eff_QTL_TT <- ggplot(L.plot.s, aes(x=LearnPowerTrans,y=Memory_Mean, color=Haplotype)) +
  geom_point(alpha = 1/3, size=1) +
  geom_point(data = effs.dat, aes(x=eLearn,y=eMem,fill=Haplotype), size=3,pch=21, color="black") +
  geom_segment(data = effs.dat, aes(x=eLearn-LearnSE, xend=eLearn + LearnSE, y=eMem, yend=eMem), color = 'black') +
  geom_segment(data = effs.dat, aes(x=eLearn, xend=eLearn, y=eMem-MemSE, yend=eMem+MemSE), color = 'black') +
