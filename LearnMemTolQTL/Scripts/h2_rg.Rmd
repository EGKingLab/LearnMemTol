...
```{r}
h2.lme(geno=as.factor(rawdLM$patRIL), pheno=rawdLM$LearnNorm)
...
x2val <- -2*logLik(afit.n, REML=TRUE) + 2* logLik(afit, REML=TRUE)
pchisq(x2val, df=1, lower.tail=F, log.p=TRUE)


#MEMORY
h2.lme(geno=as.factor(rawdLM$patRIL), pheno=rawdLM$MemoryNorm)
