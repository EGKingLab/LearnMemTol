...
ThermTol.peak <- ci.peak[[3]]
ThermTol.peak$pval <- getP(ThermTol.peak$LL,Nrils, 7, Nrils-8)
ThermTol.peak$R2 <- LOD_R2(ThermTol.peak$LL, Nrils)
...
TTqns <- paste(rep("Q",8),seq(1,8), sep="")
...
TTqns <- paste(rep("Q",7),seq(1,7), sep="")


TTq.mat <- data.frame("QTL" = TTqns, stringsAsFactors = FALSE)
TTq.mat$Gaxis <- NA
TTq.mat$LOD <- NA


for(nn in 1:length(TTqns))
{
    TTq.mat$Gaxis[nn] <- ci.peak[[3]][nn,'Gaxis']
    TTq.mat$LOD[nn] <- ci.peak[[3]][nn,'LL']
    
  
}

TTq.mat$Pval <- getP(TTq.mat$LOD,Nrils,7,Nrils-8)
...
obs.TT_long <-gather(obs.LL, key='Trait', value='LOD', Tolsqrtvariable)
obs.TT_long$Pval <- getP(obs.TT_long$LOD,Nrils,7,Nrils-8)
TTtop <- max(obs.TT_long$Pval)
...
p1 <- ggplot(obs.LM_long, aes(x=Gaxis, y=Pval, color=Trait)) +
  ...
  geom_segment(x=-50, xend=275,y=pfdr,yend=pfdr, color='grey30', linewidth=line.ww) +
  ...
  geom_line(alpha=0.6, linewidth=line.ww) +
  ...
  scale_x_continuous(breaks=c(0,66.3,120,174,221,277,33.15,98.3,145,205,249),
                     ...
                     limits = c(-0.01,max(obs.LM_long$Gaxis)+25))+
  theme(axis.ticks.x=element_blank())+
  ylab(expression("-log"[10]*"(P-value)"))+
  xlab("Position (cM)") +
  scale_color_discrete(name='Phenotype',
                       breaks=c('LearnPowerTrans','Memory_Mean'), 
...
p2 <- ggplot(obs.TT_long, aes(x=Gaxis, y=Pval)) +
  geom_rect(xmin=66.3,ymin=-10,xmax=120.3,ymax=TTtop+10,fill='aliceblue',color='aliceblue')+
  geom_rect(xmin=174,ymin=-10,xmax=221,ymax=TTtop+10,fill='aliceblue',color='aliceblue')+
  geom_segment(x=-50, xend=285,y=pfdr,yend=pfdr, color='grey30', linewidth=0.5) +
  annotate("text",label= "FDR", x=295, y=pfdr - 1, color='grey30',size=3) +
  annotate("text",label= "FWER", x=295, y=pfwer + 1, color='grey30',size=3) +
  geom_segment(x=-50, xend=285,y=pfwer,yend=pfwer, color='grey30', linewidth=0.5, lty=3) +
  geom_line(alpha=0.6, linewidth=line.ww, color='black') +
  geom_point(data = TTq.mat, aes(x=Gaxis, y=Pval+2), pch = 25, color="grey50", fill = "grey50") +
  geom_text(data=TTq.mat, mapping=aes(x=Gaxis, y=Pval+4, label = QTL), size=3, color="grey50")+
  scale_x_continuous(breaks=c(0,66.3,120,174,221,277,33.15,98.3,145,205,249), 
                     labels=c(0,"66  0",54,"108  0",47,103,'\nX','\n2L','\n2R','\n3L','\n3R'),limits = c(-0.01,max(obs.TT_long$Gaxis)+25))+
  theme(axis.ticks.x=element_blank())+
  ylab(expression("-log"[10]*"(P-value)"))+
  xlab("Position (cM)") +
  my_theme

ggsave(p2, filename = "../Plots/TT_QTL.pdf", height=4,width=10)

#make composite Fig 3

f3b <- readRDS("../../LearnMemTolPheno/Plots/Fig3b.Rds")

f3c <- readRDS("../Plots/Fig3c.Rds")

fall <- p2/(f3b + f3c) + 
  plot_annotation(tag_levels = 'A', tag_suffix = ".") &
  theme(plot.tag = element_text(size = 10))

ggsave(filename= "../Plots/Fig3.pdf", plot= fall, width= 7.5, height= 5)
