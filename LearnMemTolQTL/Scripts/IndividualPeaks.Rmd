...
TTcon_sig_genes <- as.data.frame(TTlrt_condition)
...
TTinter_sig_genes <- as.data.frame(TTlrt_inter)
...
TTcon_gene_list_merged <- merge(TTcon_sig_genes, gene_map_table, by="FBgn")
TTpool_gene_list_merged <- merge(TTpool_sig_genes, gene_map_table, by="FBgn")
TTinter_gene_list_merged <- merge(TTinter_sig_genes, gene_map_table, by="FBgn")


TTcon_gene_list_sub <- TTcon_gene_list_merged[,c(1,3,6,7,9,13:15)]
colnames(TTcon_gene_list_sub)

TTpool_gene_list_sub <- TTpool_gene_list_merged[,c(1,3,6,7,9,13:15)]
colnames(TTpool_gene_list_sub)

TTinter_gene_list_sub <- TTinter_gene_list_merged[,c(1,3,6,7,9,13:15)]
colnames(TTinter_gene_list_sub)
...
all.equal(TTpool_gene_list_sub$FBgn, TTcon_gene_list_sub$FBgn)
all.equal(TTinter_gene_list_sub$FBgn, TTcon_gene_list_sub$FBgn)
all.equal(TTpool_gene_list_sub$FBgn, TTinter_gene_list_sub$FBgn)
...
#condition
  tcon <- (which(TTcon_gene_list_sub$chr==foc.peak$chr & 
                 ((TTcon_gene_list_sub$startp <= foc.peak$upR6 & TTcon_gene_list_sub$stopp >= foc.peak$lpR6))))  
  
  TTcon_genes_under_peak<-TTcon_gene_list_sub[tcon,]
  TTcon_genes_under_peak <- TTcon_genes_under_peak[,c(1,5:8,2:4)]
  colnames(TTcon_genes_under_peak)[6:8] <- c("l2F_con","pval_con","padj_con")
  
  TTcon_genes_under_peak$l2F_pool <- TTpool_gene_list_sub[tcon,"log2FoldChange"]
  TTcon_genes_under_peak$pval_pool <- TTpool_gene_list_sub[tcon,"pvalue"]
  TTcon_genes_under_peak$padj_pool <- TTpool_gene_list_sub[tcon,"padj"]
  
  TTcon_genes_under_peak$l2F_inter <- TTinter_gene_list_sub[tcon,"log2FoldChange"]
  TTcon_genes_under_peak$pval_inter <- TTinter_gene_list_sub[tcon,"pvalue"]
  TTcon_genes_under_peak$padj_inter <- TTinter_gene_list_sub[tcon,"padj"]
  ThermTol.list[[rowN]] <-TTcon_genes_under_peak
  
}

save(ThermTol.list,file="../ProcessedData/TT_genes_under_peak.rda")
...
save(ThermTol.list,file="../ProcessedData/ThermTol_genes_under_peak.rda")
...
#####
#####
####Thermal Tolearnace Peaks

###Q1 Doesn't have any sig diff expressed genes under peak.
```{r}

###
###Libby can these three steps be done in one step?

#Q2 Condition 

ll <- 2
cc.ll <- ci.peak[[3]][ll,]
ll.sub <- ThermTol.list[[ll]]

ll.sub[which.max(abs(ll.sub$l2F_con)),]
ll.sub[which.min(ll.sub$padj_con),]

ll.sub[which(ll.sub$padj_con <= 0.05),]

unique(ll.sub[which(ll.sub$padj_con <= 0.05),]$gname)
...
ll.sub[which.max(abs(ll.sub$l2F_pool)),]
ll.sub[which.min(ll.sub$padj_pool),]

ll.sub[which(ll.sub$padj_pool <= 0.05),]

unique(ll.sub[which(ll.sub$padj_pool <= 0.05),]$gname)
...
ll.sub[which.max(abs(ll.sub$l2F_inter)),]
ll.sub[which.min(ll.sub$padj_inter),]

ll.sub[which(ll.sub$padj_inter <= 0.05),]

unique(ll.sub[which(ll.sub$padj_inter <= 0.05),]$gname)
...
ll.sub <- ll.sub[which(is.na(ll.sub$startp)==FALSE),]
ll.sub[which.max(abs(ll.sub$l2F_inter)),]
ll.sub[which.min(ll.sub$padj_inter),]
...
ll.sub[which.max(abs(ll.sub$rThermTol)),]
ll.sub[which.max(abs(ll.sub$corEff)),]

ll.sub[ll.sub$gname=="Mvl",]
ll.sub[ll.sub$gname=="Dhc93AB",]
ll.sub[ll.sub$gname=="Calx",]
ll.sub[ll.sub$gname=="Atpalpha",]

```

There are 21 genes located within this CI. 6 DE for pool, 8 for condition and 0 for interaction. 

Genes of interest *Mvl, Dhc93AB, Calx, Atpalpha*, they are all invloved in ion channels. (?)

*Mvl* and *Dhc93AB* are both DE in pool and condition.

*Mvl*
FC: `r subset(ll.sub, gname=="Mvl")$l2F_con[1]`
pval: `r subset(ll.sub, gname=="Mvl")$padj_con[1]`
pheno Therm: `r max(abs(subset(ll.sub, gname=="Mvl")$rThermTol))`

FC: `r subset(ll.sub, gname=="Mvl")$l2F_pool[1]`
pval: `r subset(ll.sub, gname=="Mvl")$padj_pool[1]`

*Mvl* encodes a member of the SLC11 family of metal ion transporters that transports divalent metal cations, including Mn2+ and Fe2+. Its function has been linked to feeding-related decisions, possibly via modulation of dopaminergic signaling.

*Dhc93AB*
FC: `r subset(ll.sub, gname=="Dhc93AB")$l2F_con[1]`
pval: `r subset(ll.sub, gname=="Dhc93AB")$padj_con[1]`

FC: `r subset(ll.sub, gname=="Dhc93AB")$l2F_pool[1]`
pval: `r subset(ll.sub, gname=="Dhc93AB")$padj_pool[1]`
pheno Therm: `r max(abs(subset(ll.sub, gname=="Dhc93AB")$rThermTol))`

*Dhc93AB* encodes a motor protein involved in hearing. 

*Calx*
FC: `r subset(ll.sub, gname=="Calx")$l2F_con[1]`
pval: `r subset(ll.sub, gname=="Calx")$padj_con[1]`

FC: `r subset(ll.sub, gname=="Calx")$l2F_pool[1]`
pval: `r subset(ll.sub, gname=="Calx")$padj_pool[1]`
pheno Therm: `r max(abs(subset(ll.sub, gname=="Calx")$rThermTol))`
...
*Atpalpha*
FC: `r subset(ll.sub, gname=="Atpalpha")$l2F_con[1]`
pval: `r subset(ll.sub, gname=="Atpalpha")$padj_con[1]`

FC: `r subset(ll.sub, gname=="Atpalpha")$l2F_pool[1]`
pval: `r subset(ll.sub, gname=="Atpalpha")$padj_pool[1]`

pheno Therm: `r max(abs(subset(ll.sub, gname=="Atpalpha")$rThermTol))`


Na pump α subunit (*Atpα*) encodes an integral membrane cation antiporter protein that utilizes ATP to shuttle Na[+] and K[+] across the plasma membrane to maintain ion homeostasis.

*....* is most significant for conditon

*.....* is most significant for pool 
