...
#Thermal Tolerance condition

TTcon_foc.peak <- ci.peak[[3]][1,]

ThermTolcon.list <-vector(mode='list', length=nrow(ci.peak$Tolsqrtvariable))

ThermTol_totdf<-data.frame(NULL)
...
for(rowN in 1:nrow(ci.peak$Tolsqrtvariable)) 
  ...
  TTcon_foc.peak<-ci.peak[['Tolsqrtvariable']][rowN,]
  tcon <- (which(TTcon_gene_list_sub$chr==TTcon_foc.peak$chr & 
  ...
  ThermTolcon.list[[rowN]] <-TTcon_genes_under_peak
  
}

save(ThermTolcon.list,file="../ProcessedData/TTcon_genes_under_peak.rda")



#Thermal Tolerance pool

TTpool_foc.peak <- ci.peak[[3]][1,]

ThermTolpool.list <-vector(mode='list', length=nrow(ci.peak$Tolsqrtvariable))
...
for(rowN in 1:nrow(ci.peak$Tolsqrtvariable)) 
  ...
  TTpool_foc.peak<-ci.peak[['Tolsqrtvariable']][rowN,]
  tpool <- (which(TTpool_gene_list_sub$chr==TTpool_foc.peak$chr & 
                 ((TTpool_gene_list_sub$startp <= TTpool_foc.peak$upR6 & TTpool_gene_list_sub$stopp >= TTpool_foc.peak$lpR6))))  
  
  TTpool_genes_under_peak<-TTpool_gene_list_sub[tpool,]
  ThermTolpool.list[[rowN]] <-TTpool_genes_under_peak
  
}

save(ThermTolpool.list,file="../ProcessedData/TTpool_genes_under_peak.rda")



#Thermal Tolerance Intereaction 

TTint_foc.peak <- ci.peak[[3]][1,]

ThermTolint.list <-vector(mode='list', length=nrow(ci.peak$Tolsqrtvariable))
...
for(rowN in 1:nrow(ci.peak$Tolsqrtvariable)) 
  ...
  TTint_foc.peak<-ci.peak[['Tolsqrtvariable']][rowN,]
  tint <- (which(TTinter_gene_list_sub$chr==TTint_foc.peak$chr & 
                 ((TTinter_gene_list_sub$startp <= TTint_foc.peak$upR6 & TTinter_gene_list_sub$stopp >= TTint_foc.peak$lpR6))))  
  
  TTinter_genes_under_peak<-TTinter_gene_list_sub[tint,]
  ThermTolint.list[[rowN]] <-TTinter_genes_under_peak
  
}

save(ThermTolint.list,file="../ProcessedData/TTinter_genes_under_peak.rda")
...
tt_effects <- merge(effects, decode.tn, by='ID',sort=FALSE, all.x=TRUE)
