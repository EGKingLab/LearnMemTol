...
pgene <- ggplot(pp.set.s[pp.set.s$sig=='N',], aes(midM, log2FoldChange, label=gname, shape=cisyn))+
  ...
  geom_text_repel(data = pp.set.s[pp.set.s$sig=='Y',], show.legend = FALSE, box.padding = 1, fontface='italic', size=txsize) +
  xlab("Position (Mbp)")+
...
pgene <- ggplot(pp.set.s[pp.set.s$sig=='N',], aes(midM, log2FoldChange, label=gname, shape=cisyn, color=Phenotype))+
  geom_segment(data=pp.set.s[pp.set.s$sig=='Y',], x=pp.set.s[pp.set.s$sig=='Y','startp']/1e6, xend=pp.set.s[pp.set.s$sig=='Y','stopp']/1e6, y=pp.set.s[pp.set.s$sig=='Y','log2FoldChange'],yend=pp.set.s[pp.set.s$sig=='Y','log2FoldChange'], color='grey50', size=2)+
  ...
  geom_text_repel(data = pp.set.s[pp.set.s$labelsig2=='Y',], show.legend = FALSE, box.padding = 1, fontface='italic', size=txsize) +
  xlab("Position (Mbp)")+
  ylab(expression("-log"[2]*"(Fold Change)")) +
  ylim(ylims)+
  scale_shape_discrete(name="cis eQTL") +
