...
ttpool.plot.list <- vector(mode='list', length=nrow(ci.peak[["Tolsqrtvariable"]])) 

for(zz in 2:nrow(ci.peak[["Tolsqrtvariable"]]))
{
  tt.set <- ThermTol.list[[zz]]
  #tt.set[which.max(abs(tt.set$FC_Pool_Overall)),]
  #tt.set[which.min(tt.set$padj_Pool_Overall),]
  
  ci.peak.set <- ci.peak[[3]][zz,]
  
  tt.set.s <- tt.set[is.na(tt.set$padj_Pool_Overall)==FALSE,]
  tt.set.s$sig <- 'N'
  tt.set.s$sig[tt.set.s$padj_Pool_Overall<=0.05]<-'Y'
  ...
  TTpool_peaks <- ggplot(tt.set.s[tt.set.s$sig=='N',], aes(midM, FC_Pool_Overall, label=labelrep))+
    geom_segment(data=tt.set.s[tt.set.s$sig=='Y',], x=tt.set.s[tt.set.s$sig=='Y','startp']/1e6, xend=tt.set.s[tt.set.s$sig=='Y','stopp']/1e6, y=tt.set.s[tt.set.s$sig=='Y','FC_Pool_Overall'],yend=tt.set.s[tt.set.s$sig=='Y','FC_Pool_Overall'], color='grey50', linewidth=segsize)+
    xlim(c(ci.peak.set$lpR6/1e6 - pbuffer, ci.peak.set$upR6/1e6 + pbuffer)) +
    geom_point(color='red2', alpha=1/6, size=2)+
    geom_point(data=tt.set.s[tt.set.s$sig=='Y',], aes(midM, FC_Pool_Overall), size=2 + 0.5, color='red2')+
    geom_hline(yintercept = 0, lty=1, color='grey80')+
    ...
    geom_text_repel(data = tt.set.s, show.legend = FALSE,  fontface='italic', size=3, segment.size=txseg.size, min.segment.length = 0, max.overlaps = 50, ylim = c(-5,0)) +
    ylab(expression("log"[2]*"(FC)")) +
    xlab(paste("Position (Mbp)", sep="")) +
    theme(legend.position="none")+
    ylim(c(-5,3))+
    annotate("label",label= paste0("Q",zz), x= ci.peak.set$lpR6/1e6, y= 2.5, color='steelblue',size=5) +
    my_theme
  
  ttpool.plot.list[[(zz)]] <- TTpool_peaks
  
}

ttpool.plot.list[[2]]

pool_all <- plot_grid(plotlist=ttpool.plot.list[2:7], labels=paste0(LETTERS[1:6],"."), ncol=2, label_size = 10)

ggsave(pool_all, filename = "../Plots/FigS6.pdf", width=6.5, height=8)


ttcon.plot.list <- vector(mode='list', length=nrow(ci.peak[["Tolsqrtvariable"]])) 
...
for(zz in 2:nrow(ci.peak[["Tolsqrtvariable"]]))
{
  ...
  tt.set.s <- tt.set[is.na(tt.set$padj_Cond_Overall)==FALSE,]
  tt.set.s$sig <- 'N'
  tt.set.s$sig[tt.set.s$padj_Cond_Overall<=0.05]<-'Y'
  tt.set.s$midM <- (tt.set.s$startp + (tt.set.s$stopp - tt.set.s$startp)/2)/1e6
  tt.set.s$labelrep <- tt.set.s$gname
  tt.set.s$labelrep[tt.set.s$sig=='N']<-""
  ...
  TTcon_peaks <- ggplot(tt.set.s[tt.set.s$sig=='N',], aes(midM, FC_Cond_Overall, label=labelrep))+
    geom_segment(data=tt.set.s[tt.set.s$sig=='Y',], x=tt.set.s[tt.set.s$sig=='Y','startp']/1e6, xend=tt.set.s[tt.set.s$sig=='Y','stopp']/1e6, y=tt.set.s[tt.set.s$sig=='Y','FC_Cond_Overall'],yend=tt.set.s[tt.set.s$sig=='Y','FC_Cond_Overall'], color='grey50', linewidth=segsize)+
    xlim(c(ci.peak.set$lpR6/1e6 - pbuffer, ci.peak.set$upR6/1e6 + pbuffer)) +
    geom_point(color='red2', alpha=1/6, size=2)+
    geom_point(data=tt.set.s[tt.set.s$sig=='Y',], aes(midM, FC_Cond_Overall), size=2 + 0.5, color='red2')+
    geom_hline(yintercept = 0, lty=1, color='grey80')+
    geom_vline(xintercept = ci.peak.set$lpR6/1e6, lty=5, color='grey80') +
    geom_vline(xintercept = ci.peak.set$upR6/1e6, lty=5, color='grey80') +
    geom_text_repel(data = tt.set.s, show.legend = FALSE,  fontface='italic', size=3, segment.size=txseg.size, min.segment.length = 0, max.overlaps = 50, ylim= c(-1.5,5)) +
    ylab(expression("log"[2]*"(FC)")) +
    xlab(paste("Position (Mbp)", sep="")) +
    theme(legend.position="none")+
    ylim(c(-1.5,6.5))+
      annotate("label",label= paste0("Q",zz), x= ci.peak.set$lpR6/1e6, y= 6, color='steelblue',size=5) +
    my_theme
  
  ttcon.plot.list[[(zz)]] <- TTcon_peaks
  
}

con_all <- plot_grid(plotlist=ttcon.plot.list[2:7], labels=paste0(LETTERS[1:6],"."), ncol=2, label_size = 10)

ggsave(con_all, filename = "../Plots/FigS7.pdf", width=6.5, height=8)
