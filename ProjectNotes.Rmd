...
3. `Validate_incapacitation.R` compares human scored and automated incapacitation scores from the heatbox. Input is `ProcessedData/ThermalTol_processed.txt` and `ProcessedData/Thermotolerance_test.csv`. and Makes a plot: `Plots/HeatBox_Human_Val.pdf` and outputs the paired data: `ProcessedData/Validation.txt` 
...
4. `genes_under_peaks.R`  looks at the differentially expressed genes from RNAseq (see below) to identify which DE genes are under each peak. And `GeneSums.Rmd` collates some of the gene summary data from FlyBase.
6. `individualpeaks.Rmd` plots each peak separately
7. `Visualize_Peak_Intervals.Rmd` plots each peak interval with the DE genes. Makes figures S6 and S7.
8. `ThermTol_additional_mapping.R` performs a genome scan after correcting for the large Q6 peak and one including subpopulation in the model to compare to our genome scan. Creates figure S2.
...
4. `Visualize_ind_genes.R` creates a series of plots of the normalized counts in each group for individual genes in the Q6 interval. Creates Figure S9.

### RNAi analysis: in `LearnMemTolPheno` folder

1. `process_raw_therm_heatplate.R` and `process_raw_therm_heatplate_val.R` processes the raw output from deeplabcut to score incapacitation for the RNAi lines and the RIL validation set. 

2. `RNAi_post_raw_process.R` checks for and corrects errors and combines the two rounds of phenotyping. 
