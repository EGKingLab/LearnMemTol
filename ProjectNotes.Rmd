...
3.  `QTL_effects_visualization_LM.Rmd` makes the plots of the haplotype effects at each QTL for learning and memory phenotypes. `TT-founder-effect.Rmd` constructs these plots for thermal tolerance data. Makes Figure 3c and Figure S (fixme)
...
7. `Visualize_Peak_Intervals.Rmd` plots each peak interval with the DE genes. [fixme]
8. `ThermTol_additional_mapping.R` performs a genome scan after correcting for the large Q6 peak and one including subpopulation in the model to compare to our genome scan. Creates figures S[fixme]
...
3. `Visualize_LMRNA_Data.Rmd` (for learning and memory) and `Visualize_TTRNA_Data.Rmd` (for thermal tolerance) create MA and volcano plots. This script creates Figure 4. 
