...
Gene2$set_id <- as.factor(Gene2$set_id)
...
test_id <- data.frame("set_id"=unique(Gene2$set_id), "simp"= c("a","b","c","d","e"))
Gene2 <- merge(Gene2, test_id, by="set_id", all=TRUE)

#run Lm 
Gene2$set_id <- as.factor(Gene2$set_id)
Gene2$simp <- as.factor(Gene2$simp)
RNAi_mod2 <- aov(incapacitation ~ simp, data= Gene2)

#shows all
glht(RNAi_mod2, linfct = mcp(simp = "Tukey"))

#set up ones we want
compare_genes <- c("a - c = 0", "b - d = 0")
glht(RNAi_mod2, linfct = mcp(simp = compare_genes))
