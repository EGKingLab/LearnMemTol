...
Learn_indi_points <- ggplot(rawdLM, aes(x = Lname, y = Learning)) +
  ...
  geom_point(data=rawdLM[rawdLM$patRIL %in% LL.rna$patRIL,], 
             aes(x=Lname, y=-1.05), 
             pch=17, alpha= 1/5, color='darkcyan') +
  geom_point(data=rawdLM[rawdLM$patRIL %in% LH.rna$patRIL,], 
             mapping=aes(x=Lname, y=-1.05), 
             pch=17, alpha= 1/5, color='darkorchid') +
