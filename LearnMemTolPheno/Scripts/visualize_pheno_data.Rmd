...
TH.rna$color <- "darkorchid"
TL.rna$color <- "darkcyan"

rna.all <- rbind(TL.rna, TH.rna)
...
ThermTol_indi_points_2 <- ggplot(rawdTT, aes(x = Tname, y = incapacitation)) +
  ...
  stat_summary(fun.data = mean_se, 
               geom = "errorbar", 
               color = 'red', width=0, size=0.3, alpha=1/2) +
  geom_point(data=rna.all, 
             aes(x=Tname, y=-10), 
             pch=17, alpha = 0.6, color=rna.all$color, size = 1) +
  geom_text_repel(data=rna.all, 
             aes(x=Tname, y= -10, label=patRIL), 
            size = 2 , direction="x", nudge_y = -100,
            angle = 270, color=rna.all$color, segment.size = 0.2, box.padding=0.25, seed = 267, max.overlaps=14) +
  theme(legend.position = "none",axis.ticks.x=element_blank()) +
  xlab("RILs") +
  ylab("Incapacitation Score (seconds)") +
  scale_y_continuous(breaks = c(0,200,400,600), limits=c(-150, 601))+
  theme(axis.text.x = element_blank()) +
  my_theme
...
#to save plot
ggsave(ThermTol_indi_points_2, file="../Plots/Fig2.pdf", width=3.5, height=3)
