...
strLearning_sort$RILorder <- 1:nrow(Learning_sort)
Learning_sort$y <- 0
...
ggplot(ThermTol_sort, aes(x = RILorder, xend = RILorder,
                        y = y, yend = Incapacitation_Mean)) +
  geom_segment(color = "blue", ) +
  geom_line(data = ThermTol_sort[ThermTol_sort$patRIL!= "12047", ]) +
  scale_x_continuous(breaks = ThermTol_sort$RILorder, labels = ThermTol_sort$patRIL) +
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5,
                                 size = 6)) +
  geom_point(aes(x = RILorder, y = Incapacitation_Mean_SE)) +
  labs(x = "Genotype", y = "Incapacitation Time (sec)") +
