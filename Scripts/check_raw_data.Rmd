...
#named LearnMem
load(file="../Data/Learn_raw.rda")


#learning and memory data processed through HeatCalc
LM_proc <- read.table(file="/home/kingeg/Projects/LearnMemTherm/HeatProc_Learn.txt", sep="\t", header=TRUE, stringsAsFactors = FALSE)


#raw thermal tolerance data
#Named ThermTol
load(file="../Data/Incapacitation.rda")
...
pA <- which(all.dat$incapacitation==0)


all.dat$flag <- rep('none',nrow(all.dat))
all.dat$flag[pA] <- 'NoAct'
...
act.ss <-which(total.set$flag=='NoAct' & is.na(total.set$Learning)==FALSE)
...
LL.set <- subset(total.set, flag=='none')
...
all.equal(TT.count$patRIL, LL.count$patRIL)
#cor(TT.count$count, LL.count$count)

#hist(TT.count$count-LL.count$count)

ss <-which(total.set$flag=='NoAct' & is.na(total.set$Learning)==FALSE)
tester <- total.set[ss,]
hist(tester$LearnAct)
hist(tester$chamber)
...
```{r}
save(ThermTol.keep,file= "../Data/Therm_good_June2018.rda")

#save(LL.set,file= "../Data/Learn_good_May2018.rda")
```
