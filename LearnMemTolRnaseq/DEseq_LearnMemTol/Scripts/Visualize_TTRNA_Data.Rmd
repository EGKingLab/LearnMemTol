...
TTgene_list <- as.data.frame(FCpool)


TTgene_list <-TTgene_list %>% mutate(FCpool = ifelse(log2FoldChange >= 2, "A", ifelse(log2FoldChange <=-2, "B", "C")))


str(TTgene_list)

TTgene_list <- drop_na(TTgene_list)

TTpool_Volcanoplot <- ggplot(TTgene_list, aes(y=-log10(padj), x=log2FoldChange)) +
