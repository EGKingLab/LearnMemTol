...
DESeq2::plotMA(TTsv_res.dds_table, ylim=c(-10,6))
...
TTgene_list <- read.table("../Data/TTDEseqSVA_resOrdered_padj.csv", header=T, sep=",")


TTgene_list <- as.data.frame(TTgene_list)


TTgene_list <-TTgene_list %>% mutate(ThermTol_Genes = ifelse(log2FoldChange >= 2, "A", ifelse(log2FoldChange <=-2, "B", "C")))


str(TTgene_list)


ThermTol_Volcanoplot <- ggplot(TTgene_list, aes(y=-log10(padj), x=log2FoldChange)) +
  geom_point(aes(colour = ThermTol_Genes), size=2, alpha=1/2 ) +
...
load(file="../Data/TTDEseqSVA_resOrdered_padj.Rda")

#use a DESeq()-ready data object
Select <- order(rowMeans(counts(TTsv_res.dds_table, normalized=TRUE)), 
