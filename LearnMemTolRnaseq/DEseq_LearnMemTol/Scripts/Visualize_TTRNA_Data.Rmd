...
Pool_Volcanoplot <- ggplot(na.omit(Poolgene_list), aes(y=-log10(padj), x=log2FoldChange, color=log10(baseMean), label=gname)) +
  ...
  ylim(c(0,75)) +
  my_theme +
  scale_color_gradientn(breaks = c(2:5), labels = 10^seq(2,5), name = "Mean\Expression",colours = rainbow_hcl(8, start = 300, end = 40)) +
  theme(legend.position="none")
...
PoolConVolPlot<- plot_grid(Pool_Volcanoplot,Con_Volcanoplot, nrow=2,labels = c('A.', 'B.'), rel_widths = c(1,1.3), label_size = 10)
