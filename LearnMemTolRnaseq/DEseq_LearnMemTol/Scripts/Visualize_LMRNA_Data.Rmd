...
DESeq2::plotMA(LMsv_dds_deseq, ylim=c(-25,25))
...
load(file="../Data/LMDEseqSVA_resOrdered_padj.Rda")

par(mar=c(5,5,5,5), cex=1.0, cex.main=1.4, cex.axis=1.4, cex.lab=1.4)

LMtop <- as.data.frame(LMres_dds_table)

#Adjusted P values (FDR Q values)
with(TTres_dds_table, plot(log2FoldChange, -log10(padj), pch=20, main="Volcano plot", cex=1.0, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~Q~value)))
...
LMgene_list <- as.data.frame(LMsv_res_dds_table)

LMgene_list <-LMgene_list%>%mutate(LMgene_list = ifelse(log2FoldChange) >= 2, "A", ifelse(log2FoldChange<=-2, "B", "C" ))


str(TTdrhs)


Learn_Volcanoplot <- ggplot(LMgene_list, aes(y=-log10(padj), x=log2FoldChange)) +
...
ggsave(Memgene_Volcanoplot, filename="../Plots/LearnMem_Volcanoplot.pdf", width=6, height=4)
