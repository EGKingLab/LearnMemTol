...
save(TTDE, file = "../Data/ThermTolddsDE.Rda")
save(TTresOrdered, file = "../Data/ThermTolres.Rda")
...
TTres.dds2 <- results(TTdds_deseq2)
(TTresOrder2 <- TTres.dds2[order(TTres.dds2$padj), ])

save(TTdds_deseq2, file = "../Data/ThermTolddsDE2.Rda")
save(TTresOrder2, file = "../Data/ThermTolresOrdered2.Rda")
```
...
TTdds.sva <- estimateSizeFactors(TTdds)
TTdat.sva <- counts(TTdds.sva, normalized=TRUE)
TTcc.c <- rowSums(TTdat.sva)

#remove all genes with 0s for all samples
TTdat.sva <- TTdat.sva[which(TTcc.c>0),]
...
TTn.sv <- num.sv(TTdat.sva, TTmod)

print(c("Calculated number of significant SVs = ", TTn.sv))

TTsvobj <- svaseq(TTdat.sva, TTmod, TTmod0, n.sv=TTn.sv) 


TTdat.sva$SV1 <- TTsvobj$sv[,1]
TTdat.sva$SV2 <- TTsvobj$sv[,2]

head(TTdat.sva, 3)

TTdat.sva <- as.data.frame(TTdat.sva)

ggplot(TTdat.sva, aes(x=SV1, y=SV2)) +
geom_point(alpha=1/5, size=3)

save(TTdat.sva, file = "../Data/TTdat.sva.Rda")
...
#add SVs to dataset
TTpheno$SV1 <- TTsvobj$sv[,1]
TTpheno$SV2 <- TTsvobj$sv[,2]

# DESeqDataSet for all variables,

TTsv_dds.ttSS <- DESeqDataSetFromMatrix(countData = TTgenes, 
                                   colData = TTpheno, 
                                   design = ~ SV1 + SV2 + batch + pool + condition)

TTsv_dds_deseq <- DESeq(TTsv_dds.ttSS, fitType = "local")
TTsv_res.dds <- results(TTsv_dds_deseq)
(TTsv_res.dds <- TTsv_res.dds[order(TTsv_res.dds$padj), ])



## Merge with normalized count data
TTsv_res.dds_table <- merge(as.data.frame(TTsv_res.dds), 
                       as.data.frame(counts(TTsv_dds_deseq, normalized=TRUE)), 
                       by="row.names", sort=FALSE)
names(TTsv_res.dds_table)[1] <- "Gene"

save(TTsv_res.dds, file = "../Data/TTsv_res.dds.Rda")
save(TTsv_dds_deseq, file = "../Data/TTsv_dds_deseq.Rda")
save(TTsv_res.dds_table, file = "../Data/TTDEseqSVA_resOrdered_padj.Rda")
...
hist(TTsv_res.dds$pvalue, breaks=50, col="grey")

# Examine independent filtering
metadata(TTsv_res.dds)$filterThreshold

plot(metadata(TTsv_res.dds)$filterNumRej, type="b", xlab="quantiles of baseMean", ylab="number of rejections")
```
