...
TTlrt_conondition<- results(TTlrt_con_deseq)

TTcon <- lfcShrink(TTlrt_con_deseq, contrast =c("condition", "Incu","RT" ), type="normal")

save(TTcon, file = "../Data/TTlrt_condition.Rda")
...
save(TTpool, file = "../Data/TTlrt_pool.Rda")

TTlrt_inter_deseq <- DESeq(TT_dds_02, test="LRT", reduced=~ SV1 + SV2 + batch + pool + condition)
TTlrt_inter <- results(TTlrt_inter_deseq)

#TTinter <- lfcShrink(TTlrt_inter_deseq, contrast =c("condition", "Incu","RT"))

save(TTlrt_inter, file = "../Data/TTlrt_inter_deseq.Rda")
...
#step 1
#get p values using likelihood ratio test for pool, condition, and interaction (done above). Make an object for each so we can get the p-values later
