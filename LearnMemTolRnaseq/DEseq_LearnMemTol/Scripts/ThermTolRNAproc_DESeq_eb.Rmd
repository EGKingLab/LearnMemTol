...
saveRDS(TTpheno, file="../Data/Pheno_data_withBatch.Rds")
...
save(TTlrt_con_deseq, file = "../Data/TTdds_condition.Rda")
...
TT_dds_02 <- DESeqDataSetFromMatrix(countData = TTgenes, 
                                   colData = TTpheno, 
                                   design = ~ SV1 + SV2 + batch + condition + pool)

TTlrt_pool_deseq <- DESeq(TT_dds_02, test="LRT", reduced=~ SV1 + SV2 + batch + condition)

save(TTlrt_pool_deseq, file = "../Data/TTdds_pool.Rda")
...
save(TTlrt_inter_deseq, file = "../Data/TTdds_inter.Rda")
...
length(which(FC_All$padj_Pool_Overall < 0.05 & FC_All$FC_Pool_Overall > 0))
length(which(FC_All$padj_Pool_Overall < 0.05 & FC_All$FC_Pool_Overall < 0))

length(which(FC_All$padj_Cond_Overall < 0.05 & FC_All$FC_Cond_Overall > 0))
length(which(FC_All$padj_Cond_Overall < 0.05 & FC_All$FC_Cond_Overall < 0))
